...
from soulstruct.havok.enums import TagDataType
from soulstruct.havok.exceptions import HavokTypeError, TypeNotDefinedError, VersionModuleError
from soulstruct.havok.types.info import TypeInfo, HAVOK_TYPE_PREFIXES
...
def create_module_from_files(version: str, *file_paths: str | Path, is_tagfile=True, module_path: Path = None):
    ...
    from soulstruct.havok.packfile.unpacker import PackFileUnpacker
    from soulstruct.havok.tagfile.unpacker import TagFileUnpacker
    for file_path in file_paths:
    ...
    if not is_tagfile:
        ...
        if version == "2010":
            from soulstruct.havok.types import hk2010 as base_module
            module_str += f"from soulstruct.havok.types.hk2010_base import *\n"
        elif version == "2014":
            from soulstruct.havok.types import hk2014 as base_module
            module_str += f"from soulstruct.havok.types.hk2014_base import *\n"
        else:
